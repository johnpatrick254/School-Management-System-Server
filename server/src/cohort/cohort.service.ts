import { Body, ConflictException, Get, Injectable, InternalServerErrorException, NotFoundException, Param, Post, Put } from '@nestjs/common';
...
export class CohortService {
  ...
  async create(data: CreateCohortDTO): Promise<Cohort> {
    try {
      ...
      if (error.code === "P2002") throw new ConflictException('cohort already exist');
      throw new InternalServerErrorException()
    }
  };

  @Put(':id')
  async update(@Param('id') id: string, @Body() data: Pick<UpdateCohortDTO, "careerId" | "code" | "name">) {
